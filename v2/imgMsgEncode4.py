...
def encode_text_to_image(text, password, img_path, debug=False):
	...
	# Only convert if image mode is not already RGB or RGBA.
	if img.mode not in ["RGB", "RGBA"]:
		img = img.convert("RGB")
	pixels = np.array(img)
	img_size = pixels.shape[0], pixels.shape[1]
	message_length = len(text)
	binary_length = format(message_length, '032b')  # 32-bit binary for message length
	...
	for i, bit in enumerate(binary_message):
		...
		row = pos // img_size[1]
		col = pos % img_size[1]
		channel = i % 3  # Modify only R, G, B channels so that alpha remains unchanged.
...
def decode_text_from_image(password, img_path, debug=False):
	...
	binary_length = ''
	for i in range(32):  # First 32 bits represent the message length.
		pos = pixel_positions[i]
		row = pos // img_size[1]
		col = pos % img_size[1]
		channel = i % 3
